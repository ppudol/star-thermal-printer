...
class Star extends PrinterType {
  ...
  setTextSize(height, width) {
    this.buffer = null;
    if (height > 5 || height < 0) throw new Error('setTextSize: Height must be between 0 and 5');
    if (width > 5 || width < 0) throw new Error('setTextSize: Width must be between 0 and 5');

    this.append(Buffer.from([0x1b, 0x69, width.toString(16), height.toString(16)]));
    return this.buffer;
  }
