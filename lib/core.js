...
class ThermalPrinter {
  ...
  parseStatusRT(status) {
    let bufferArray = [...status]

    if (this.readBit(3, bufferArray[5])) {
      return this.errors.PAPER_END
    }

    if (this.readBit(2, bufferArray[5])) {
      return this.errors.PAPER_NEAR_END
    }

    return this.errors.NONE;
  }

  readBit(bitPosition, byte) {
    if (bitPosition < 0 || bitPosition > 7) {
      throw new RangeError("Bit position must be between 0 and 7");
    }

    const mask = 1 << bitPosition;
    return (byte & mask) >> bitPosition;
  }

  async getErrors() {
    return this.Interface.getResponse(this.printer.config.STATUS_RT)
      .then((data) => {
        return (this.parseStatusRT(data));
      })
      .catch((error) => {
        return this.errors.CONNECTION_ERROR
      })
  }
